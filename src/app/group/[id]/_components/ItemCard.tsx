    ...
    <div className="relative w-full" >
      ...
      >
        ...
        <div className="flex flex-col">
          <span className="font-medium text-lg truncate">{item.content} {item.username}</span>
          {/* ✅ Conteneur de l'heure et de l'icône avec effet barré */}
          <div className={`flex items-center gap-2 ${item.state === 1 ? "line-through opacity-50" : ""}`}>
        ...
        <div>
          {item.state === 1 && <span className="text-secondary text-lg font-bold">✔️</span>} {/* Terminé */}
          {item.state === 2 && <span className="text-success text-lg font-bold">🟢</span>} {/* En cours */}
        </div>
      </div>
    </div>
  );
